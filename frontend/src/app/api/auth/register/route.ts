...
export async function GET() {
  ...
  // kc_action=register tells Keycloak to show the registration form by default if supported
  const authorizationUrl = `${keycloakUrl}/protocol/openid-connect/auth?client_id=${clientId}&redirect_uri=${encodeURIComponent(
    redirectUri
  )}&response_type=code&scope=openid+email+profile&kc_action=register`;
